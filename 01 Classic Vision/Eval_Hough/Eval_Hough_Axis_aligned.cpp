...
int main()
{
	std::string test_name = "canny_val_med5_mask_best_AA";
    ...
    Evaluator test_evaluator("../../03 Data/Simple Dataset", "../../02 Deep Learning/Create-CSV-dataset/list_of_img_in_val_set_18-03.csv", 0); // Last parameter determines the prediction target
    //Evaluator test_evaluator("../../03 Data/Dataset2_onPallet", "../../02 Deep Learning/Create-CSV-dataset/list_of_img_in_OP_val_set_14-04.csv", 0);
	...
	std::vector<int> canny_lows = { 48 };//{ 15,18,21,24,27,30,33,36,39,42,45,48,51,54,57,60};//
	std::vector<int> canny_hysts = { 84 }; // { 30,33,36,39,42,45,48,51,54,57,60,63,66,69,72,75,78,81,84,87,90,93,96,99,102,105,108,111,114,117,120,123,126,129,132,135,138,141,144,147,150,153,156,159,162,165};//
	...
	for (int i = 0; i < canny_lows.size(); i++)
	{
		...
		for (int j = 0; j < canny_hysts.size(); j++)
		{
			...
			dump << setw(sp) << "IoU" << setw(sp) << "Prec." << setw(sp) << "Rec." << setw(sp) << "F1" << setw(sp)
				<< "ang. err." << setw(sp) << "a. e. std." << setw(sp) << "cent. err." << setw(sp) << "c. err. std" << setw(sp) << "F. Neg" << setw(sp) << "F. Pos" << setw(sp) << "T. Pos" << std::endl;
