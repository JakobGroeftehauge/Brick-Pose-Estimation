...
int main(int argc, const char* argv[])
{
    CommandLineParser cmd(argc, argv,
        "{ image i        | pic2.png  | input image }"
        "{ template t     | templ1.png | template image }"
        "{ full           | 1          | estimate scale and rotation }"
        "{ gpu            |           | use gpu version }"
        "{ minDist        | 100       | minimum distance between the centers of the detected objects }"
        "{ levels         | 360       | R-Table levels }"
        "{ votesThreshold | 2500      | the accumulator threshold for the template centers at the detection stage. The smaller it is, the more false positions may be detected }"
        ...
        "{ minScale       | 0.8       | minimal scale to detect }"
        "{ maxScale       | 1.2         | maximal scale to detect }"
        "{ scaleStep      | 0.1      | scale step }"
        "{ minAngle       | 0         | minimal rotation angle to detect in degrees }"
        "{ maxAngle       | 1       | maximal rotation angle to detect in degrees }"
        "{ angleStep      | 1         | angle step in degrees }"
        "{ maxBufSize     | 10000      | maximal size of inner buffers }"
    ...
    cv::Mat brick_templ = cv::imread("brick_template.jpg", IMREAD_COLOR);
    cv::cvtColor(brick_templ, brick_templ, CV_BGR2GRAY);
    //cv::medianBlur(brick_templ, brick_templ, 5);
    //cv::Canny(brick_templ, brick_templ, 60, 180);
    //cv::bitwise_not(brick_templ, brick_templ);

    cv::imshow("template", brick_templ);

    cv::Mat brick_color = cv::imread("../Strojer_Images/Initial Test Images/Cropped/IMG_4047.JPG", IMREAD_COLOR);
    cv::cvtColor(brick_color, image, CV_BGR2GRAY);
    //cv::medianBlur(image, image, 5);
    //cv::Canny(image, image, 60, 140);
    ...
    if (!full)
    {
        Ptr<GeneralizedHoughBallard> ballard = useGpu ? cv::createGeneralizedHoughBallard() : createGeneralizedHoughBallard();
    ...
    {
        Ptr<GeneralizedHoughGuil> guil = useGpu ? cv::createGeneralizedHoughGuil() : createGeneralizedHoughGuil();
