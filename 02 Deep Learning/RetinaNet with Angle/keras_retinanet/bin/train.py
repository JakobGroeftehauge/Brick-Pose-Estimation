...
def print_loss_to_csv(train_loss, val_loss, path, name='losses.csv'):
...
def model_with_weights(model, weights, skip_mismatch):
    ...
    if weights is not None:
        model.load_weights(weights, by_name=True, skip_mismatch=skip_mismatch)  #load_weights is a keras method.
                                                                                #by_name is true to only load layers with same name, i.e the backbone
    return model
...
def main(args=None):
    ...
    print_history_to_csv(HPM_train_model.history, '02 On Pallet Dataset/', '_losses.csv')
